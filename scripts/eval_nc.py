    ...
    for seg_id in [i for i in np.unique(gt_seg_0) if i != 0]:
        if (gt_seg_0 == seg_id).sum() < 20:
            ...
            for _ in range(100):
                ...
                _nc_scores.append(get_miou(gt_seg_0 == seg_id, segmentation_masks))
            nc_scores_0.append(sum(_nc_scores)/len(_nc_scores))
    ...
    for seg_id in [i for i in np.unique(gt_seg_1) if i != 0]:
        if (gt_seg_1 == seg_id).sum() < 20:
            ...
            for _ in range(100):
                ...
                segmentation_masks = (seg_maps[:, sampled_point[0]:sampled_point[0]+1, 
                                    sampled_point[1]:sampled_point[1]+1] == seg_maps)
                _nc_scores.append(get_miou(gt_seg_1 == seg_id, segmentation_masks))
            nc_scores_1.append(sum(_nc_scores)/len(_nc_scores))
    ...
    for seg_id in [i for i in np.unique(gt_seg_2)]:
        if (gt_seg_2 == seg_id).sum() < 20:
            ...
            for _ in range(100):
                ...
                segmentation_masks = (seg_maps[:, sampled_point[0]:sampled_point[0]+1, 
                                    sampled_point[1]:sampled_point[1]+1] == seg_maps)
                _nc_scores.append(get_miou(gt_seg_2 == seg_id, segmentation_masks))
            nc_scores_2.append(sum(_nc_scores)/len(_nc_scores))
